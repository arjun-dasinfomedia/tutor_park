...
const GuardiansParents = (props) => {
  ...
  const loadSessionsDynamic = getUserData()
    .parents.filter((item) => {
      if (props.SearchData === "") {
        return item;
      } else if (
        item.name === null
          ? ""
          : item.name.toLowerCase().includes(props.SearchData.toLowerCase())
      ) {
        return item;
      } else if (
        item.email === null
          ? ""
          : item.email.toLowerCase().includes(props.SearchData.toLowerCase())
      ) {
        return item;
      } else if (
        item.tp_id === null
          ? ""
          : item.tp_id.toLowerCase().includes(props.SearchData.toLowerCase())
      ) {
        return item;
      }
    })
