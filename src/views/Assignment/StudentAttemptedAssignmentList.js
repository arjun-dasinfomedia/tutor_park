...
import { faEye, faSearch, faCircle } from "@fortawesome/free-solid-svg-icons";
...
const studentAttemptedAssignment = () => {
  ...
  const pageCount = Math.ceil(
    assignmentState.assignmentList.attempted_assignments &&
      assignmentState.assignmentList.attempted_assignments.length / PER_PAGE
  );
  const offset = currentPage * PER_PAGE;
  const [loadAssignmentDataStudent, setloadAssignmentDataStudent] =
    useState("");
      ...
      assignmentState.assignmentList.attempted_assignments
        .filter((item) => {
          if (searchAssignment === "") {
            ...
            item.syllabus === null ??
            item.syllabus.toLowerCase().includes(searchAssignment.toLowerCase())
          ) {
            return item;
          } else if (
            ...
            item.subject === null ??
            item.subject.toLowerCase().includes(searchAssignment.toLowerCase())
          ) {
            return item;
          } else if (
            item.class === null ??
            item.class.toLowerCase().includes(searchAssignment.toLowerCase())
          ) {
            return item;
          }
          {
                        ...
                        <div className="col-12 justify-content-center d-flex d-md-block justify-content-lg-center justify-content-md-center justify-content-sm-center">
                          <div className="carddetailfooter normal-font">
                            {item.tutor_status === undefined ? (
                              ...
                              <CBadge color="primary" className="h6">
                                Evaluated
                              </CBadge>
                            )}
                          </div>
      ...
      <div>
        {isLoading ? (
          <>{loader}</>
            ...
            undefined &&
          assignmentState.assignmentList.attempted_assignments !== null ? (
          <>
            {StudentAssignemtAssignmentData}
            <div className="mt-4 col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12 d-flex justify-content-center text-center">
              {assignmentState.assignmentList.attempted_assignments.length >
              10 ? (
                <ReactPaginate
                  previousLabel={"<<"}
                  nextLabel={">>"}
