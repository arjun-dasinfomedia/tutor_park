...
const Messages = () => {
  ...
  const showchatID = (item) => {
    ...
    dispatch(
      messageRead({ conversation_id: item.conversation_id, module: "message" })
    );
  };

  const sendDirectChatAction = (email) => {
    if (message.personalMessageListData !== null) {
  ...
  useEffect(async () => {
    showLoader();
    await dispatch(messageList({ module: "message" }));
    await dispatch(messagefriendData());
    setLoading(false);
    hideLoader();
  }, []);
  ...
  const directChatDynamic =
    message.personalMessageListData &&
    message.personalMessageListData
      .filter((item) => {
        if (searchchatname == "") {
          return item;
    ...
    <>
        ...
        <div style={{ marginTop: "15px" }}>
          ...
          <div className="d-flex mt-3 mb-3">
            ...
            >
              <Conversation
                Data={chatData}
                HistroyData={message.histroyListData}
                Responsive={show}
              />
            </div>
          </div>
      ...
      {value === 1 ? (
        ...
        >
          ...
          <CModalBody>
            <MaterialTable
              ...
              data={message.directmessagefriendData}
              actions={[
                {
                  icon: "chat",
        ...
        >
          <CModalHeader
            ...
            className="tutorviewmodalheader"
          >
            <CModalTitle>Group Message</CModalTitle>
          </CModalHeader>
          <CModalBody>
            <div className="row">
      ...
      >
        ...
        <CModalBody>
          <MaterialTable
            title={groupname}
            data={message.groupmessagefriendData}
