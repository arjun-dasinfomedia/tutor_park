...
const NetworkStudent = (props) => {
  ...
  const pageCount = Math.ceil(studentDataArray.length / PER_PAGE);

  const offset = currentPage * PER_PAGE;

  const loadfriendlistDynamic = studentDataArray
    .filter((item) => {
      if (props.SearchData == "") {
        return item;
      } else if (
  ...
  return (
    <div>
      {isLoading ? (
        ...
        <NoDataContainer module="Student List" />
      )}
    </div>
  );
};
export default NetworkStudent;
