...
const TimeLineList = (prop) => {
    ...
    useEffect(async () => {
        showLoader();
        await dispatch(getOtherTimeLineList());
    ...
    // Repost Timeline
    const repostTimeline = (data) => {
        Swal.fire({
            title: "Are you sure?",
            text: "You Want to Repost this timeline!",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    dispatch(repostTimeLine({ timeline_id: data }))
                }
            });
    }

    // render Abusse And UnAbuse Action Menu
                                ...
                                <CCard className="friendcard p-3 ">
                                    ...
                                    >
                                        ...
                                        <div className="ml-2 ">
                                            ...
                                            {
                                                checkAccessPermission('timeline_edit') ? <><div className="d-inline r-dot-menu position-absolute ml-4">
                                                        ...
                                                        <CDropdown variant="nav-item" className="d-md-inline marker-remove-textbook-css mr-3">
                                                            ...
                                                            <CDropdownMenu className="pt-0 course-action-dropdown-menu-css m-2" placement="bottom-end">
                                                                ...
                                                                {item.is_reposted !== true ?
                                                                    (<CDropdownItem
                                                                        onClick={() => repostTimeline(item.id)}
                                                                    >
                                                                        Timeline Repost
                                                                    </CDropdownItem>)
                                                                    : ""
    ...
    return (
        <div>
            {isLoading ? (
                <>{loader}</>
            ) : loadOtherTimeLineListData.length !== 0 ? (
                <div>
